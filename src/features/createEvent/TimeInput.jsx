...
export default function TimeInput({ label, type, id, placeholder, element,inputHandler,value,empty,emptyHandler}) {
  return (
    <div className="grid gap-2 w-full">
      <label
        htmlFor={id}
        className="text-lg inline-flex items-center lg-screen:items-start font-normal lg-screen:font-semibold text-[#2D2C3C] capitalize"
      >
        {label}
      ...
      <div className="relative">
        ...
        <input
          ...
          required
        />
         {empty && <p className="text-red-600 absolute z-0 ">{label} is required</p>}
      </div>
    </div>
  );
}
